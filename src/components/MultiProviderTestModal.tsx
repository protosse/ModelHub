...
export function MultiProviderTestModal({
  ...
  // Empty by default: backend injects per-protocol client defaults per request.
      ...
      <div className="mb-3">
        ...
        {showHeaders ? (
          ...
          <p className="truncate rounded-md border border-surface-3 bg-surface-1 px-2 py-1.5 font-mono text-[11px] text-ink-3">
            自动 {protocolAutoHeadersSummary()}
            {headerOverrideLines(headersText).length > 0
              ? ` · 全局覆盖 ${headerOverrideLines(headersText).join(" · ")}`
              : ""}
          </p>
        )}
      </div>
